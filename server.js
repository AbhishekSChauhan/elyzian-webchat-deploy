...
app.get('/',(req,res)=>{
  console.log('coming to root');
  res.status(200).send('Succeses');
})
...
app.get('/js/webchat/:companyId', (req, res) => {
  ...
  const filePath = path.join(__dirname, 'public', 'js', 'webchat.js');
   console.log('in company id', companyId);
  ...
  if (fs.existsSync(filePath)) {
    ...
    fs.readFile(filePath, 'utf8', (err, data) => {
      ...
      const modifiedData = data.replace(/650c64be035ef7dd6769dbc0/g, companyId);
...
app.get('/js/signup/:companyId', (req, res) => {
  const companyId = req.params.companyId;
  const filePath = path.join(__dirname, 'public', 'js', 'signup.js');
   console.log('in company id', companyId);
  // Check if the file exists
  if (fs.existsSync(filePath)) {
    // Read the file
...
// Route to serve the minified CSS file
app.get('/css/signup', (req, res) => {
  // const filename = req.params.filename;
  const filePath = path.join(__dirname, 'public', 'css', 'signup.css');
